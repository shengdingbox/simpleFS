...
import com.shengdingbox.blog.business.dto.BizCommentDTO;
...
import com.shengdingbox.blog.util.UrlBuildUtil;
...
public class BizCommentServiceImpl implements BizCommentService {
    ...
    public void commentForAdmin(Comment comment) throws DaoBaoCommentException {
    ...
    @Override
    @RedisCache(flush = true)
    public Comment comment(Comment comment) throws DaoBaoCommentException {
        ...
        if (!anonymous && !SessionUtil.isLogin()) {
            throw new DaoBaoCommentException("站长已关闭匿名评论，请先登录！");
        }
    ...
    private void filterContent(Comment comment) {
        ...
        if (StringUtils.isEmpty(content) || "\n".equals(content)) {
            throw new DaoBaoCommentException("说点什么吧");
        }
        // 过滤非法属性和无用的空标签
        if (!XssKillerUtil.isValid(content) || !XssKillerUtil.isValid(comment.getAvatar())) {
            throw new DaoBaoCommentException("请不要使用特殊标签");
        }
    ...
    private void setCurrentAnonymousUserInfo(Comment comment) {
        if (StringUtils.isEmpty(comment.getNickname())) {
            throw new DaoBaoCommentException("必须输入昵称");
        }
    ...
    public void doSupport(Long id) {
        ...
        if (redisTemplate.hasKey(key)) {
            throw new DaoBaoCommentException("一个小时只能点一次赞哈~");
        }
    ...
    public void doOppose(Long id) {
        ...
        ValueOperations<String, Object> operations = redisTemplate.opsForValue();
        if (redisTemplate.hasKey(key)) {
            throw new DaoBaoCommentException("一个小时只能踩一次哈~又没什么深仇大恨");
        }
