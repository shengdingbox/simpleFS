...
public class SysLinkServiceImpl implements SysLinkService {
    ...
    public boolean autoLink(Link link) throws DabaoLinkException {
        String url = link.getUrl();
        Link bo = getOneByUrl(url);
        if (bo != null) {
            throw new DabaoLinkException("本站已经添加过贵站的链接！");
        }
        ...
        if (!(LinksUtil.hasLinkByHtml(url, domain))
            ...
            throw new DabaoLinkException("贵站暂未添加本站友情链接！请先添加本站友链后重新提交申请！");
        }
