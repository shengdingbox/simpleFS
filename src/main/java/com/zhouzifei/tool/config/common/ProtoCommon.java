...
package com.zhouzifei.tool.config.common;
...
public class ProtoCommon {
   ...
   * @return token string
   */
  public static String getToken(String remote_filename, int ts, String secret_key) throws UnsupportedEncodingException, NoSuchAlgorithmException, ServiceException {
  ...
  public static String genSlaveFilename(String master_filename,
                                        String prefix_name, String ext_name) throws ServiceException {
    String true_ext_name;
    int dotIndex;

    if (master_filename.length() < 28 + FDFS_FILE_EXT_NAME_MAX_LEN) {
      throw new ServiceException("master filename \"" + master_filename + "\" is invalid");
    }
    ...
    if (true_ext_name.length() == 0 && prefix_name.equals("-m")) {
      throw new ServiceException("prefix_name \"" + prefix_name + "\" is invalid");
    }
